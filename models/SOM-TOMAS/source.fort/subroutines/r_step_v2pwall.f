C     =====================================================================================
C           THIS SUBROUTINE STEPS FORWARD FOR VAPOR COND. ON WALL-DEPOSITED PARTICLES
C     =====================================================================================
      
      SUBROUTINE STEP_V2PWALL(Pres,Temp,RH,BoxVol,alpha,k_f,k_r,Db,
     &                        V2PWL,dt)
      ...
      DOUBLE PRECISION alpha,BoxVol,Pres,Temp,RH,storg
...
cdbg      integer numcalls          !number of times cond routine is called
      ...
      DOUBLE PRECISION Db         ! particle-phase diffusion coefficient [m2/s]
      ...
      DOUBLE PRECISION psatorg(iMONO) !(iORG)
      DOUBLE PRECISION T1
...
C      PARAMETER(kcc = 0.01)
      ...
      IF (V2PWL.EQ.0) RETURN

      do k=1,ibins
         if (Nk_wall(k) .lt. Nepss) then
            Nk_wall(k)=Nepss
            do j=1,icomp
               Mk_wall(k,j)=0.d0
            enddo
            Mk_wall(k,xSO4)=Nepss*1.4*xk(k) !make the added particles SO4 !xSO4 is an index that points to the position of SO4
         endif                          !xSO4 = 2
      enddo

      do k=1,ibins
         do j=1,icomp-idiag
            if (Mk_wall(k,j) .eq. 0.d0) then
               !add a small amount of mass
               mtotal=0.d0
               do jj=1,icomp-idiag
                  mtotal=mtotal+Mk_wall(k,jj)
               enddo
               Mk_wall(k,j)=1.d-10*mtotal
            endif
         enddo
      enddo
...
C-----CODE--------------------------------------------------------------
      ...
      Mass_ORG = SUM(Mk_wall(:,xORG_1ST:xORG_LST))
...
C      DO i = 1,iCOMP
C         IF (PARNAME(i).EQ.'GENSOMG_10_04') THEN
C            xC10O4 = i-1
C            GOTO 201
C         END IF
C      END DO
...
C      PRINT*, PARNAME(2+xC10O4-1)
C      STOP
      
      DO j = 1,iMONO !iORG
C         IF ((MASS_ORG*1e9/(BoxVol*1e-6).LT.1.).AND.(j.EQ.xC10O4)) THEN
C            CONTINUE
C            psatorg(j)=pstar_ref(iORG-2)*
C     &           exp(Hvap(j)*1.d3*(1.d0/T1-1.d0/temp)/R)
C            Csat_Org(j) = cstar_ref(iORG-2)
C         ELSE
            psatorg(j)=Psat_Ref(j)*
     &           EXP(Hvap(j)*1.d3*(1.d0/T1-1.d0/temp)/R)
            Csat_Org(j) = Csat_Ref(j)
C         END IF
      END DO

C      DO j = 1,iMONO
C         PRINT*, j,PARNAME(xORG_1ST+j-1),Csat_Org(j)
C      END DO
C      STOP


      
C      IF (MASS_ORG*1e9/(BoxVol*1e-6).LT.5.) THEN

C         DO j = 1,iorg
         !psatorg(j)=pstar_ref(j)*exp(Hvap(j)*1.d3*(1.d0/T1-1.d0/temp)/R)
C            psatorg(j)=pstar_ref(iORG-2)*
C     &      exp(Hvap(j)*1.d3*(1.d0/T1-1.d0/temp)/R)
C         ENDDO
...
C     &         Mkf(k,xH2O))
            ...
            density = CALC_AEDENS(orgmass,Mk_wall(k,1))
...
C         Db = 1.0e-10          ! particle-phase diffusion coefficient [m2/s]
C         kc = 0.0               ! first-order loss rate of species in the particle phase [1/s]
         
C        CALCULATE PARTICLE AND CORE RADII:
         Vol_ORG   = SUM(Mk_wall(k,xORG_1ST:xORG_LST))/Nk_wall(k)/1400.
         Vol_INORG = Mk_wall(k,1)/Nk_wall(k)/1770.
         Vol_TOT   = Vol_ORG + Vol_INORG
         
         Rp = (3.0*Vol_TOT/4.0/PI)**(1./3.)
         Rc = (3.0*Vol_INORG/4.0/PI)**(1./3.)

C        CALCULATE REACTION RATE (KC) OF COND. SPECIES:
         Vol_COND = Vol_ORG*Nk_wall(k)*1e6
         Num_COND = 0.0

CCCCC         
         DO i = 1,iMONO-1
            Num_COND = Num_COND + 
     &          Mk_wall(k,xORG_1ST+i-1)/(ORG_MW(i)/1000.0)*NA/Vol_COND
         END DO
         
         IF (jo.EQ.iMONO) THEN
            kc = 0.0
         ELSE
            kc = k_f*Num_COND - k_r*Mk_wall(k,xDIME_1ST+jo-1)/
     &                              Mk_wall(k,xORG_1ST+jo-1)
            IF (kc.LT.0.0) kc = 0.0
...
C Set dp for nonvolatile species
      DO k = 1,iBINS
         ...
         TOTPHASE = SUM(Mk_wall(k,xORG_1ST:xORG_LST)) + 
     &              Mk_wall(k,xSO4)*NONORGSCALE
     ...
     &                     /(8.314*temp*1200.*Dpk(k))) !! kelvin effect, mw dependent
         ...
         if (totphase.gt.0.d0) then
            psat=psatorg(jo)*Mkf(k,j)/totphase*scalefactor/QBULK(k) ! This is Raoult's law
C            psat=psatorg(jo)*(Mkf(k,j)+Mkf(k,xDIME_1ST+jo-1))/
C     &      totphase*scalefactor ! This is Raoult's law
C            PRINT*, 'MONO=',Mkf(k,j),'DIMER=',Mk_wall(k,xDIME_1ST+jo-1)
C            psat=psatorg(jo)*(Mkf(k,j)+Mk_wall(k,xDIME_1ST+jo-1))
C     &                /totphase*scalefactor ! This is Raoult's law

            !psat=psatorg(iORG-2)*Mkf(k,j)/totphase*scalefactor ! This is Raoult's law
...
!         endif
        ...
        if (totphase .EQ. 0.0) then
         ...
         masseqm=pamb*QBULK(k)/(psatorg(jo)/totphase*scalefactor)
C         masseqm=pamb/(psatorg(jo)/totphase*scalefactor)
          
         !masseqm=pamb/(psatorg(iORG-2)/totphase*scalefactor)
...
C      cdt = dt/10.0
      ...
      do k=1,ibins
         ...
         if (isnan(sK)) then
            ...
            PRINT*,'Vol_COND=',Vol_COND,'Nk=',Nk_wall(k)
            PRINT*, 'Vol_ORG=',Vol_ORG,'Mk=',Mk_wall(k,:)
C            print*,'tk=',tk
C            print*,'tj=',tj
C            print*,'dp=',dp
C            print*,'cdt=',cdt
